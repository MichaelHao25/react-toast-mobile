...
import Snap from 'snapsvg'; // class DialogCustom extends React.Component {
//     static show = params => {
...
let svg, path, progress, timerId, load, success, fail; // ToastContainer.prototype.toast = tips => {
//     let container = document.createElement("div");
//     document.body.appendChild(container);
//     function closeHandle() {
//         ReactDOM.unmountComponentAtNode(container);
...
const Message = props => {
  ...
  }, React.createElement("span", null, props.tips)));
}; //不能直接加载成功!
//不能加载成功后在加载失败!
...
const Loading = props => {
  ...
  return React.createElement("div", {
    className: `react_toast_mask react_toast_mask_loading ${text != '' && "active"}`
  }, React.createElement("div", {
    className: "react_toast_message"
...
const App = () => {
  return React.createElement("div", null, React.createElement(LoadAnimation, null));
};
...
_defineProperty(ToastContainer, "toast", tips => {
  let container = document.createElement("div");
  document.body.appendChild(container);
...
_defineProperty(ToastContainer, "load", tips => {
  load(tips);
});

_defineProperty(ToastContainer, "success", tips => {
  success(tips);
});

_defineProperty(ToastContainer, "fail", tips => {
  fail(tips);
});